...
const { author } = Reply;
  ...
  if (!global.tictactoe) global.tictactoe = new Map();
  let data = global.tictactoe.get(threadID);
  ...
  if(!isNaN(number) && number > 0 && number < 10) {
    ...
    if(checkGameOver(data)) {
      var gayban = ["I won baka","try again baka","learn how to play"];
      if(checkAIWon(data)) lmao = `you lost! ${gayban[Math.floor(Math.random() * gayban.length)]}`;
      else if(checkPlayerWon(data)) lmao = "I won baka";
      else lmao = "Lucky, but next time we won't draw !";
      global.tictactoe.delete(threadID);
    }
    var msg = lmao !== "" ? lmao : temp == undefined ? "Reply to the message with the number of fthe square " : temp;
  ...
  } else return api.sendMessage("This is the number of your dreams", threadID, messageID);
},

onStart: async function ({ event, api, args, threadsData, commandName }) {
  if (!global.tictactoe) global.tictactoe = new Map();
  let { threadID, messageID, senderID } = event;
  const threadSetting = await threadsData.get(threadID) || {};
  var prefix = global.GoatBot.config.PREFIX;
  let data = global.tictactoe.get(threadID) || { "gameOn": false, "player": "" };
  let concak = "" + prefix + commandName;
  let newData;
  if (args.length == 0) return api.sendMessage("Choose X or O", threadID, messageID);
  if (args[0].toLowerCase() == "end") {
    global.tictactoe.delete(threadID);
    return api.sendMessage("The game has ended!", threadID, messageID);
  }
  if (args[0].toLowerCase() == "continue") {
    if (!data.gameOn) return api.sendMessage("No data! use" + concak + " x/o to start over", threadID, messageID);
    return api.sendMessage({ body: "Reply with the number of boxes", attachment: await displayBoard(data)}, threadID, (error, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        author: senderID,
        messageID: info.messageID
  ...
  if (!data.gameOn) {
    ...
    if (abc !== "x" && abc !== "o") return api.sendMessage("Choose X or O", threadID, messageID);
    if (abc == "o") {
      newData = startBoard({ isX: false, data, threadID });
      api.sendMessage({ body: "Reply with the number of the square you want to play with", attachment: await displayBoard(newData)}, threadID, (error, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName,
          author: senderID,
          messageID: info.messageID
    ...
    if (abc == "x") {
      ...
      AIStart(newData);
      api.sendMessage({ body: "The bot plays first!\nReply with the number of the square", attachment: await displayBoard(data)}, threadID,(error, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName,
          author: senderID,
          messageID: info.messageID
    ...
    global.tictactoe.set(threadID, newData);
  } else return api.sendMessage("A game has been created in this group\nUse:\n ttt2 continue -> pick up where you left off\nttt2 end -> end the game", threadID, messageID);
    }
};
...
async function displayBoard(data) {
  const path = __dirname + "/tmp/ttt.png";
  let canvas = createCanvas(1200, 1200);
  let cc = canvas.getContext("2d");
  let background = await loadImage("https://i.imgur.com/AvCSvCn.png");
  cc.drawImage(background, 0, 0, 1200, 1200);
  var quanO = await loadImage("https://i.postimg.cc/7PmvR6DW/20230513-144946.png");
  var quanX = await loadImage("https://i.postimg.cc/qMP1DVtF/20230513-145147.png");
  for (var i = 0; i < 3; i++) {
    for (var j = 0; j < 3; j++) {
